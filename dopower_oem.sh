...
iostat -dxmct 5 ${disk} > ${timestamp}/pre/${disk}.iostat &
...
pkill -9 iostat
sudo pkill -9 sfx-status

##大压力顺序写IO，运行脚本如下：
iostat -dxmct 5 ${disk} > ${timestamp}/seqw/${disk}.iostat &
...
##大压力随机写IO，运行脚本如下：
iostat -dxmct 5 ${disk} > ${timestamp}/randw/${disk}.iostat &
...
##大压力混合随机写IO，运行脚本如下：
iostat -dxmct 5 ${disk} > ${timestamp}/randmix_rw/${disk}.iostat &
